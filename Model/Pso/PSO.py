...
class PSO(SkoBase):
    ...
    def __init__(self, func, dim=4, pop=40, max_iter=150, lb=None, ub=None, w=0.8, c1=0.5, c2=0.5):
        ...
        self.gbest_y = 0 # general best image  for all particles in history
    ...
    def update_pbest(self):
        ...
        :return:
        '''
        if self.pbest_y.max() < self.Y.max():
            self.pbest_y= self.Y
            self.pbest_x= self.X
        '''   
        self.pbest_x = np.where(self.pbest_y.max() < self.Y.max, self.X, self.pbest_x)
