...
for root, dirs, files in os.walk("D:\PycharmWorkspace\Cooperation\Hucongcong\GWAS\\", topdown=False):
    for file in files:
        fp = open("D:\PycharmWorkspace\Cooperation\Hucongcong\GWAS\\non-Gwas_in_snp_new"+".txt","w")
        fp1 = open("D:\PycharmWorkspace\Cooperation\Hucongcong\GWAS\\non-Gwas_in_snp_ref" + ".txt", "w")
        for line in open("F:\Genebentness\GWAS\\non-Gwas_in_snp.txt"):
            target_fragment=''
            target_chrom = line.split("\t")[1]
            if target_chrom[3:] not in chrom_list:
                print(target_chrom)
                continue
            count+=1
            chromSEQ = genomic[target_chrom]
            target_start = int(line.split("\t")[2])
            target_end = int(line.split("\t")[3])
            target_distance = target_end-target_start
            target_shift = (-target_start+target_end)//2
            source_base = line.split("\t")[8]
            convert_base = line.split("\t")[9].split("/")
            if 'lengthTooLong' in convert_base:
                continue
            target_style = line.split("\t")[11]

            if source_base in convert_base:
                convert_base.remove(source_base)
            target_sp_fragment = line.split("\t")[8]
            ...
            if target_style =="insertion":
                ...
                for num in convert_base:
                    ...
                    fp.write(str(target_chrom) + "\t" + str(target_start) + "\t" + str(
                        target_end) + "\t" + target_style + "\t" + line.split("\t")[0] + "\t" + str(num) + "\t")
                    fp.write(target_fragment + "\n")
